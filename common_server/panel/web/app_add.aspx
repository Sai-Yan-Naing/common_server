...
protected string appcmd(string user)
{
	...
	string physicalPath = @"E:\webroot\LocalUser\" + user + @"\web" + path.Replace("/", "\\");
	if ( physicalPath.Contains("&") || physicalPath.Contains("..")) {
		return "";
	}
    ...
    // 引数：アプリケーションへの変換
    p.StartInfo.Arguments = @"add app /site.name:" + user + @" /path:" + path + " /physicalPath:" + physicalPath;
    ...
    // 引数：アプリケーションにアプリケーションプールを設定
    p.StartInfo.Arguments = @"set app " + user + path + " /applicationPool:" + user;
    p.Start();
    p.WaitForExit();
...
<% Response.Redirect("~/app_list.aspx?is_app=4&sub_dir=" + c); %>
